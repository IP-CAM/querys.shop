...
class ControllerBlogArticle extends Controller {
      ...
      if (!$this->model_blog_helper->checkDbTable('blog_setting')) { $this->redirect($this->url->link('common/home')); }
		...
		if ($articleData) {
         ...
         if ($articleData['meta_title']) {
            $this->document->setTitle($articleData['meta_title']);
         } else{
         ...
         $imageHeight = $blogSetting['articleFeatHeight'];
            ...
            $totalComments = $this->model_blog_article->getTotalCommentsByArticleId($this->request->get['article_id']);
               ...
               if (isset($this->request->get['category_id'])) {
                  $artCommByCatId = in_array($this->request->get['category_id'], $commentCatDisabled);
                     if ($artCommByCatId) {
                        $artComment = $this->model_blog_article->getCommentByArticleToCategory($artCommByCatId);
                     } else {
                        $artComment = '';
                     }
               } elseif ($catDatas) {
                  $commCatIdDatas = $this->model_blog_article->getCategoriesId($article_id);
            ...
            if ((array_intersect($catDisabled, $commentCatDisabled) && $articleData['comment'] == '0') || $artComment == '2') {
					$commentStatus    = 0;
					$heading_comment  = '';
					$comments         = '';				
				} else {
         ...
         if(isset($blogSetting['blogArtTemplate'])) {
            ...
            if (isset($blogArtTemp[$this->config->get('config_store_id')])) {
               $blogArtTemplate  = $blogArtTemp[$this->config->get('config_store_id')];
            }
         }
         
         $articleTemplate     = $this->model_blog_article->getArticleTemplate($article_id);
         $templateFolder      = $this->config->get('config_template') . '/template/blog/';

         if (file_exists(DIR_TEMPLATE . $templateFolder . 'article.tpl')) {
         ...
         if ($articleTemplate != 'default') {
            ...
            if (file_exists(DIR_TEMPLATE .$templateFolder . 'article_' . $articleTemplate . '.tpl')) {
               $this->template = $templateFolder . 'article_' . $articleTemplate . '.tpl';
            } elseif (file_exists(DIR_TEMPLATE . $templateFolder . 'article_' . $blogArtTemplate . '.tpl')) {
               $this->template = $templateFolder . 'article_' . $blogArtTemplate . '.tpl';
            }
         }
		...
		foreach ($results as $result) {
			...
			foreach ($replyDatas as $replyData) {
               ...
               'badgeColor'   => $badgeReplyColor,
					'name'			=> $replyData['name'],
					'avatar'			=> $this->model_blog_helper->getGravatar($replyData['email'], $blogSetting['commentAvatar']),
	...
	public function write() {
      ...
      if ($this->registry->has('shortcodes')) { // Check if registry conatain shortcodes and strip user input
