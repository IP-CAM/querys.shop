...
class ModelBlogArticle extends Model {
   ...
   public function getRelatedProduct($article_id) {
      $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_product_related bpr LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (bpr.product_id = p2s.product_id) WHERE bpr.article_id = '" . (int)$article_id . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
   ...
   public function getArticleTemplate($article_id) {
      $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_article_to_template WHERE article_id = '" . (int)$article_id . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "'");
   ...
   public function getCustomerGroup($customer_id) {
      ...
      if ($this->model_blog_helper->checkDbColumn('customer_group', 'name')) {
   ...
   public function getCategoriesId($article_id) {
      $query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "blog_article_to_category WHERE article_id = '" . (int)$article_id . "' ORDER BY category_id ASC LIMIT 1");

      return $query->row['category_id'];
   }
   
   public function getCommentByArticleToCategory($category_id) {
      $query = $this->db->query("SELECT comment FROM " . DB_PREFIX . "blog_article a LEFT JOIN " . DB_PREFIX . "blog_article_to_category a2c ON (a.article_id = a2c.article_id) WHERE a2c.category_id = '" . (int)$category_id . "'");
   ...
   public function getTags($limit = 20) {
      ...
      if ($this->model_blog_helper->checkDbColumn('product_description', 'tag')) { // if exist mean >= 1.5.4
         // Exclude product tags due the OpenCart v1.5.4 database change!
