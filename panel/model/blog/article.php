...
class ModelBlogArticle extends Model {
	public function addArticle($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "blog_article SET author_id = '" . (int)$data['author_id'] . "', image = '" . $this->db->escape($data['image']) . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', comment = '" . (int)$data['comment'] . "', available = '" . $this->db->escape($data['available']) . "', created = '" . $this->db->escape($data['created']) . "', modified = '" . $this->db->escape($data['modified']) . "'");

		$article_id = $this->db->getLastId(); 
			
		foreach ($data['article_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "blog_article_description SET article_id = '" . (int)$article_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', description = '" . $this->db->escape($value['description']) . "'");
		}
      ...
      if (isset($data['article_template'])) {
			foreach ($data['article_template'] as $store_id => $template) {
				if ($template) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "blog_article_to_template SET article_id = '" . (int)$article_id . "', store_id = '" . (int)$store_id . "', template = '" . $this->db->escape($template['template']) . "'");
				}
			}
		}
	...
	public function editArticle($article_id, $data) {
      ...
      $this->db->query("DELETE FROM " . DB_PREFIX . "blog_article_to_template WHERE article_id = '" . (int)$article_id . "'");
	...
	public function copyArticle($article_id) {
		...
		if ($query->num_rows) {
         ...
         $data['comment']     = '2';
			...
			$data = array_merge($data, array('article_layout' => $this->getArticleLayoutsCopy($article_id)));
			$data = array_merge($data, array('article_template' => $this->getArticleTemplatesCopy($article_id)));
	...
	public function getArticles($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "blog_article a LEFT JOIN " . DB_PREFIX . "blog_article_description ad ON (a.article_id = ad.article_id)";

			if (!empty($data['filter_category'])) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "blog_article_to_category a2c ON (a.article_id = a2c.article_id)";			
			}
         
         if ($data['sort'] == 'a2c.category_id') {
				$sql .= " LEFT JOIN " . DB_PREFIX . "blog_article_to_category a2c ON (a.article_id = a2c.article_id)";			
			}
         
         if ($data['filter_store'] != 'all') {
				$sql .= " LEFT JOIN " . DB_PREFIX . "blog_article_to_store a2s ON (a.article_id = a2s.article_id)";			
			}
         
         if ($data['sort'] == 'a2s.store_id') {
				$sql .= " LEFT JOIN " . DB_PREFIX . "blog_article_to_store a2s ON (a.article_id = a2s.article_id)";			
			}
   ...
   public function getArticleLayoutsCopy($article_id) {
		$article_layout_data_copy = array();
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_article_to_layout WHERE article_id = '" . (int)$article_id . "'");
		
		foreach ($query->rows as $result) {
			$article_layout_data_copy[$result['store_id']] = array('layout_id'   => $result['layout_id']);
		}
		
		return $article_layout_data_copy;
	}
	
   public function getArticleTemplates($article_id) {
		$article_template_data = array();
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_article_to_template WHERE article_id = '" . (int)$article_id . "'");
		
		foreach ($query->rows as $result) {
			$article_template_data[$result['store_id']] = $result['template'];
		}
		
		return $article_template_data;
	}
   
   public function getArticleTemplatesCopy($article_id) {
		$article_template_data_copy = array();
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_article_to_template WHERE article_id = '" . (int)$article_id . "'");
		
		foreach ($query->rows as $result) {
			$article_template_data_copy[$result['store_id']] = array('template'  => $result['template']);
		}
		
		return $article_template_data_copy;
	}
