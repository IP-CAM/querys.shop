...
class ControllerBlogArticle extends Controller {
	...
	private function getForm() {
      ...
      if (isset($this->request->post['comment'])) {
			...
			$this->data['comment'] = $article_info['comment'];
		} else {
			$this->data['comment'] = '';
		}
      ...
      $this->load->model('design/layout');
		...
		if (isset($this->request->post['article_layout'])) {
			foreach ($this->request->post['article_layout'] as $store_id => $layout) {
            $this->data['article_layout'][$store_id] = $layout['layout_id'];
			}
		} elseif (isset($article_info)) {
      ...
      $storeTemplates = $this->model_blog_article->getStoresTemplate();
      foreach( $storeTemplates as $storeTemplate) {
         $templateData     = '';
         $templateFiles    = glob(DIR_CATALOG . 'view/theme/' . $storeTemplate['value'] . '/template/blog/article_*.tpl');
         ...
         if ($templateFiles) {
            foreach ($templateFiles as $templateFile) {
               ...
               $template1  = str_replace('article_', '', $template);
      ...
      if (isset($this->request->post['article_template'])) {
			foreach ($this->request->post['article_template'] as $store_id => $template) {
            $this->data['article_template'][$store_id] = $template['template'];
			}
		} elseif (isset($article_info)) {
			$this->data['article_template'] = $this->model_blog_article->getArticleTemplates($article_info['article_id']);
		} else {
			$this->data['article_template'] = array();
		}
