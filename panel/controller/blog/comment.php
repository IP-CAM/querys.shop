...
class ControllerBlogComment extends Controller {
	...
	private function getList() {
		...
		$comment_total = $this->model_blog_comment->getTotalComments($data);
		...
		foreach ($results as $result) {
			...
			$this->data['comments'][] = array (
				...
				'filter_email'  => $this->url->link('blog/comment', 'token=' . $this->session->data['token'] . $url . '&filter_email=' . $result['email'], 'SSL'),
				'filter_name'  => $this->url->link('blog/comment', 'token=' . $this->session->data['token'] . $url . '&filter_name=' . $result['name'], 'SSL'),
				'in_reply'		=> $parentName,
				'name'			=> $result['name'],
				'email'			=> $result['email'],
