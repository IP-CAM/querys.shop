...
class ControllerBlogSetting extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateSetting()) {
            ...
            $blogHomeTemplate       = (isset($this->request->post['blogHomeTemplate'])) ? $this->request->post['blogHomeTemplate'] : array();
            $blogCatTemplate        = (isset($this->request->post['blogHomeTemplate'])) ? $this->request->post['blogCatTemplate'] : array();
            $blogArtTemplate        = (isset($this->request->post['blogHomeTemplate'])) ? $this->request->post['blogArtTemplate'] : array();
            ...
            $post_data['blogHomeTemplate']         = serialize($blogHomeTemplate);
            $post_data['blogCatTemplate']          = serialize($blogCatTemplate);
            $post_data['blogArtTemplate']          = serialize($blogArtTemplate);
		...
		// Fallback compatibility and transition for Blog home multi-language. Removed when BM reach v.1.4.2
		$this->data['blogNameValue'] = '';
		$this->data['blogTitleValue'] = '';
      ...
      //== Blog Home: Specific Template
      $this->specificTemplate('Home', 'home_category_');
      
      if (isset($this->request->post['blogHomeTemplate'])) {
			$this->data['blogHomeTemplateValues'] = $this->request->post['blogHomeTemplate'];
		} elseif (isset($this->data['blogHomeTemplateValue'])) {
			$this->data['blogHomeTemplateValues'] = unserialize($this->data['blogHomeTemplateValue']);
		} else {
			$this->data['blogHomeTemplateValues'] = array();
		}
      
      //== Categories: Default Template
      $this->specificTemplate('Cat', 'category_');
      
      if (isset($this->request->post['blogCatTemplate'])) {
			$this->data['blogCatTemplateValues'] = $this->request->post['blogCatTemplate'];
		} elseif (isset($this->data['blogCatTemplateValue'])) {
			$this->data['blogCatTemplateValues'] = unserialize($this->data['blogCatTemplateValue']);
		} else {
			$this->data['blogCatTemplateValues'] = array();
		}
      
      //== Articles: Default Template
      $this->specificTemplate('Art', 'article_');
      
      if (isset($this->request->post['blogArtTemplate'])) {
			$this->data['blogArtTemplateValues'] = $this->request->post['blogArtTemplate'];
		} elseif (isset($this->data['blogArtTemplateValue'])) {
			$this->data['blogArtTemplateValues'] = unserialize($this->data['blogArtTemplateValue']);
		} else {
			$this->data['blogArtTemplateValues'] = array();
		}
	...
	private function specificTemplate($identifier = 'Blog', $tempPrefix = 'blog_') {
      $this->data['templates' . $identifier] = array();
      $storeTemplates = $this->model_blog_category->getStoresTemplate();
      foreach ($storeTemplates as $storeTemplate) {
         $templateData     = '';
         $templateFiles    = glob(DIR_CATALOG . 'view/theme/' . $storeTemplate['value'] . '/template/blog/' . $tempPrefix . '*.tpl');
         
         if ($templateFiles) {
            foreach ($templateFiles as $templateFile) {
               $template   = basename($templateFile, '.tpl');
               $template1  = str_replace($tempPrefix, '', $template);
            ...
            $this->data['templates' . $identifier][$storeTemplate['store_id']] = $templateData;
         }
      }
   }
